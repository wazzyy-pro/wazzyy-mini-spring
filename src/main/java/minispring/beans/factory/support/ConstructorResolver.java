...
import minispring.beans.SimpleTypeConverter;
import minispring.beans.TypeConverter;
import minispring.beans.factory.config.BeanDefinition;
import minispring.beans.factory.config.DependencyDescriptor;
import minispring.core.DefaultParameterNameDiscoverer;
    ...
    public Object[] resolveConstructorArguments(String beanName, BeanDefinition beanDefinition,
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            try {
                ...
                throw new BeansException("无法解析构造函数参数: " + parameter.getName() +
                        " 类型: " + parameter.getType().getName() + "对于Bean: " + beanName, e);
            }
        }
        return args;
    }
